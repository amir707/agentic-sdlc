...
def test_transient_error_detection_and_backoff():
    ...
    assert _is_transient(gemini_429)
    assert 10 <= _retry_seconds(gemini_429, 0) <= 11  # provider hint + margin

    anthropic_429 = Exception("RateLimitError: rate_limit_error ...")
    assert _is_transient(anthropic_429)
    assert _retry_seconds(anthropic_429, 1) == 30.0   # exponential fallback

    # capacity flakes retry too (killed a cloud run before they did)
    assert _is_transient(Exception(
        "503 UNAVAILABLE. This model is currently experiencing high demand."))
    assert _is_transient(Exception("529 overloaded_error"))

    assert not _is_transient(Exception("400 INVALID_ARGUMENT"))
