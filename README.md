...
| `projects-config/` | **one folder per governed project**: `project.yaml` (repo, areas, smoke endpoints), `pipeline.yaml` (the pipeline *shape* — today `human_gate: true\|false`; guarantees are not knobs), `backlog.json`, `.env` (gitignored), and `steps/<step>/` overlays mirroring `sdlc/steps/` |
