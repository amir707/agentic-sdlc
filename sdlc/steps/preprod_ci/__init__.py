...
def run_preprod(pr: int, checkout_dir: str, areas: set[str],
                project, deployer) -> CIResult:
    """`deployer` is the Deployer port (sdlc.ports.world) — the engine
    injects it; this step never names gcloud."""
    ...
    preprod_url = deployer.deploy_preprod(pr, str(checkout))
