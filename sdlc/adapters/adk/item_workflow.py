...
the per-item execution path. `sdlc/definition.py` remains the
...
engine's own node decisions (`sdlc/sprint/pipeline.py`: policy, loop
budgets, governance outcomes, routes — all framework-free), and
`sdlc/adapters/adk/executor.py` runs it on ADK's engine. The definition's
