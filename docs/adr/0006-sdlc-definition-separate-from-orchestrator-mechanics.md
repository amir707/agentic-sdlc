...
- `sdlc/definition.py` — WHAT the process is: the SDLC as a
...
- `sdlc/bindings.py` — HOW it executes: a sequential,
  inspectable driver with an explicit HANDLERS registry binding each
  definition step name to its `sdlc/steps/<name>/` implementation.
- `orchestrator/` utilities, `sdlc/tools/`, and `adapters/` — the mechanisms both lean on
...
Customizing the SDLC = edit the definition, add an `sdlc/steps/`
